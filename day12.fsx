...
let filterNextCaves smallCaveFilter (routeResults: Cave list list) (route: Cave list) (nextCave : Cave) = 
    ...
    then false
    elif smallCaveFilter nextCave route
    then false
...
let partOneFilter nextCave route = nextCave.Size = CaveSize.Small && (route |> List.contains nextCave)
let partTwoFilter nextCave route = 
    nextCave.Size = CaveSize.Small
    && (route |> List.contains nextCave)
    && (route 
        |> List.filter (fun f -> f.Size = CaveSize.Small) 
